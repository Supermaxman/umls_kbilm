...
class RelationCollator(object):
	def __init__(self, tokenizer, example_creator: RelationExampleCreator, max_seq_len: int, negative_sample_size: int, force_max_seq_len: int):
...
class UmlsRelationDataModule(pl.LightningDataModule):
	def __init__(
			self, data_folder: str, umls_directory: str, batch_size: int, num_workers: int, collator: RelationCollator):
		super().__init__()
		self.data_folder = data_folder
		self.umls_directory = umls_directory
	...
	def prepare_data(self, *args, **kwargs):
		# check if cache file exists for dataset pickled examples.
		cache_file = os.path.join(self.data_folder, 'umls_cache.pickle')
		if not os.path.exists(cache_file):
			# if file does not exist then create it
			load_umls(self.umls_directory, self.data_folder)

	def setup(self, stage=None):
		self.concepts, self.relation_types, self.relations = load_umls(self.umls_directory, self.data_folder)
